...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, attention_mask, token_type_ids, label_id, e1_mask, e2_mask, center_list):
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        dis = (e22_p-e11_p + 1) / len(tokens)
        if dis > 0.5:
            for i in range(len(tokens)):
                center_list[i] = random.uniform(0.3, 0.5)
            for i in range(e11_p, e22_p + 1):
                center_list[i] = random.uniform(0.9, 1.1)
        else:
            for i in range(len(tokens)):
                center_list[i] = random.uniform(0.9, 1.1)
            for i in range(e11_p, e22_p + 1):
        ...
        if ex_index < 5:
            ...
            logger.info("center_list: %s" % " ".join([str(x) for x in center_list]))
